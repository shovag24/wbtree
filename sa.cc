...
double SA_Floorplan(WB_Tree &fp, int k, int local, float term_T)
{
  ...
  pre_cost =  best = fp.calculate_cost();
  ...
  do{
   ...
   for(; uphill < N && MT < 2*N; MT++){
       ...
       if(d_cost <=0 || rand_01() < p ){
         ...
         // keep best solution
         if(cost < best){
           fp.keep_sol(fp.best_sol);
           //fp.show_tree();
           best = cost;
           printf("   ==>  Cost= %f, Area= %.6f, ", best, fp.Area*1e-6);
           printf("Wire= %.3f\n", fp.WireLength*1e-3);
           assert(fp.Area >= fp.TotalArea);
           time = seconds();  
         }
       }
       else{
         reject++;
         fp.recover(fp.last_sol);
       }
   }
...
//   cout << T << endl;
  ...
  fp.recover(fp.best_sol);
  cout<<"recover last : "<<endl;
  //fp.show_tree();
  fp.self_aligned_packing();
  fp.calculate_cost();
  return time; 
}
